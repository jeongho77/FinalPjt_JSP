...
<body>
	...
	ArrayList<ReplyDto> replyList = dao2.Know_Reply_list(number);
	dao.know_view(number);
	...
	<main class="container">
		<section class="main-list">
			...
			<div class="modify-button">
   				 <button class="changeBtn w-btn" type="button" onclick="location.href='../knowledge_update.jsp?number=<%=dto.getNumber()%>'">수정</button>
    			 <button class="changeBtn w-btn" type="button" onclick="location.href='../knowledge_delete.jsp?number=<%=dto.getNumber()%>'">삭제</button>
			</div>
			<%
			}
			%>
			...
			<div class="like-button">
				<button class="w-btn" type="button" onclick = "location.href='http://localhost:8080/Pjt/project/board_page/knowledge_like.jsp?number=<%=dto.getNumber()%>'">좋아요!</button>
				<p><%=dto.getLike() %></p>
			</div>
			...
			<ul class="comment-list">
				...
				<li style="position: relative;">
					...
					%>
					<button style="width:20px; height: 20px; border: none; position: absolute; transform: translateY(-50%); top: 70%; right: 0%" type="button" onclick="location.href='../knowledge_reply_delete.jsp?number=<%=reply.getNumber()%>&com_num=<%=dto.getNumber()%>'">
